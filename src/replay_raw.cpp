...
const char* DEFAULT_TOPIC_OUT = "ultrasound_raw_data_replay";
const char* DEFAULT_GCS_PREFIX = "ultrasound/raw";
const int DEFAULT_FRAME_INTERVAL_MS = 20;
const uint64_t DEFAULT_DEVICE_ID = 9001;

volatile sig_atomic_t running = 1;

void signalHandler(int /*signum*/) {
    std::cout << "\n[ReplayRaw] Shutdown signal received..." << std::endl;
    running = 0;
}
...
std::vector<GcsObject> list_gcs_csv_objects(
    ...
    if (!run_command_capture(cmd, output_lines, rc)) {
        std::cerr << "[ReplayRaw] Failed to execute gsutil ls command." << std::endl;
        return objects;
    }
    ...
    if (objects.empty()) {
        std::cerr << "[ReplayRaw] No CSV files found under " << pattern << std::endl;
    }
    if (rc != 0 && objects.empty()) {
        std::cerr << "[ReplayRaw] gsutil ls returned exit code " << rc << std::endl;
    }
...
bool copy_gcs_to_local(const std::string& uri, const std::string& local_path, const std::string& project_id) {
    ...
    if (rc != 0) {
        std::cerr << "[ReplayRaw] Failed to download " << uri << " (exit code " << rc << ")" << std::endl;
        return false;
    }
    return true;
}
...
std::vector<std::vector<uint8_t>> read_csv_rows(const std::string& csv_path) {
    ...
    if (!input.is_open()) {
        std::cerr << "[ReplayRaw] Failed to open " << csv_path << std::endl;
        return {};
    }
    ...
    while (std::getline(input, line)) {
        ...
        if (!all_cells_valid) {
            ...
            std::cerr << "[ReplayRaw] Non-numeric CSV data at line " << line_no << " in " << csv_path << std::endl;
            return {};
        }
...
rd_kafka_t* create_producer(const std::string& brokers) {
    ...
    if (!rk) {
        std::cerr << "[ReplayRaw] Failed to create producer: " << errstr << std::endl;
...
bool publish_frame(
    ...
    header.device_id = static_cast<uint32_t>(device_id);
    header.num_samples = static_cast<uint32_t>(frame.size());
    header.sample_format = 8;
    std::memset(header.reserved, 0, sizeof(header.reserved));
    header.payload_length = static_cast<uint32_t>(frame.size());

    std::vector<uint8_t> message;
    message.reserve(sizeof(USFrameHeader) + frame.size());
    const uint8_t* h_ptr = reinterpret_cast<const uint8_t*>(&header);
    message.insert(message.end(), h_ptr, h_ptr + sizeof(USFrameHeader));
    ...
    if (rd_kafka_produce(
            ...
            0,
            nullptr) != 0) {
        std::cerr << "[ReplayRaw] Failed to publish frame: " << rd_kafka_err2str(rd_kafka_last_error()) << std::endl;
        return false;
    }
    return true;
}
...
int main() {
    ...
    const std::string bucket = get_env_or_default("REPLAY_GCS_BUCKET", get_env_or_default("RAW_GCS_BUCKET", ""));
    ...
    const std::string local_dir = get_env_or_default("REPLAY_LOCAL_DIR", "/tmp/replay_raw");
    ...
    if (bucket.empty()) {
        std::cerr << "[ReplayRaw] Missing REPLAY_GCS_BUCKET (or RAW_GCS_BUCKET fallback)." << std::endl;
        return 1;
    }

    std::filesystem::create_directories(local_dir);

    std::cout << "[ReplayRaw] Brokers: " << brokers << std::endl;
    std::cout << "[ReplayRaw] GCS source: gs://" << bucket << "/" << prefix << std::endl;
    std::cout << "[ReplayRaw] Topic out: " << topic_out << std::endl;

    const auto objects = list_gcs_csv_objects(bucket, prefix, project_id);
    const auto selected = select_objects(objects, file_pattern);
    if (selected.empty()) {
        std::cerr << "[ReplayRaw] No replay files selected." << std::endl;
        return 1;
    }

    if (file_pattern.empty()) {
        std::cout << "[ReplayRaw] Default mode: replaying most recent file " << selected.front().uri << std::endl;
    } else {
        std::cout << "[ReplayRaw] Selected " << selected.size() << " file(s) matching pattern '" << file_pattern << "'" << std::endl;
    }
    ...
    if (!topic) {
        std::cerr << "[ReplayRaw] Failed to create topic handle for " << topic_out << std::endl;
    ...
    do {
        for (const auto& object : selected) {
            ...
            std::cout << "[ReplayRaw] Downloading " << object.uri << std::endl;
            ...
            if (frames.empty()) {
                std::cerr << "[ReplayRaw] No frames decoded from " << local_file << std::endl;
                std::error_code rm_ec;
                std::filesystem::remove(local_file, rm_ec);
                continue;
            }

            std::cout << "[ReplayRaw] Publishing " << frames.size()
                      << " frame(s), samples/frame=" << frames.front().size() << std::endl;

            int published = 0;
            for (const auto& frame : frames) {
                if (!running) break;
                if (publish_frame(topic, frame, sequence++, device_id, next_ts_ns)) {
                    ++published;
            ...
            std::cout << "[ReplayRaw] Published " << published << " frame(s) from " << object.uri << std::endl;

            std::error_code rm_ec;
            std::filesystem::remove(local_file, rm_ec);
            if (rm_ec) {
                std::cerr << "[ReplayRaw] Failed to remove temporary file " << local_file
    ...
    std::cout << "[ReplayRaw] Done." << std::endl;
    return 0;
}
